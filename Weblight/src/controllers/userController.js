...
const register = async function (req, res) {
  try {
   ...
   // console.log(body,"1")
    ...
    const { fname, lname, email, phone, address ,role} = body;
    ...
    if (role) {
      if (!validator.isValid(role)) {
          return res.status(400).send({ status: false, message: "Please enter role in valid format" });
      }
      if (!["customer", "admin"].includes(isPublished)) {
          return res.status(400).send({ status: false, message: "role  should be either 'customer' or 'admin'" });
      }
  }
  body["role"] = role;
...
const login = async function (req, res) {
  try {
    ...
    let token = jwt.sign(payload, "Weblight");
